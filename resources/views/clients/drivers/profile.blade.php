            ...
            <div class="col col-sm-5">
                <div class="card mb-3">
                    ...
                    <div class="card-body text-center">
                        <img src="@if ($driver->photo == null)
                        {{ url('images/default-driver.png') }}
                        @else
                        {{ url('images/'.$driver->photo) }}
                        @endif" alt="Christina Mason" class="img-fluid rounded-circle mb-2" width="128" height="128" />
                        <h5 class="card-title mb-0">{{ $driver->name }}</h5>
                        <div class="text-muted mb-2">
                            @if (count($driver->cars))
                                {{ $driver->cars[0]->model }} ({{ $driver->cars[0]->license }})
                            @else
                                No car
                            @endif
                        </div>
